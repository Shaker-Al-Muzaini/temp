...
const ImagesList = () => {
  ...
  function updateData() {
    photoshootService.getPhotoshootImagesList(params.id).then((result) => {
  ...
  function deleteImage(code) {
    console.log(code)
    photoshootService.deleteImage(code).then((result) => {
      if (result) {
        updateData()
      }
    })
    ...
    <CCol xs={12}>
      <CCard className="mb-4">
        <CCardHeader>
          ...
          <br></br>
          <CButton
            onClick={() => navigate("/photoshoot/post-images-add/"+params.id)}
            color={'primary'}
          >
