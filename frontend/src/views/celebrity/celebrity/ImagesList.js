...
const ImagesList = () => {
  ...
  function updateData() {
    celebritiesService.getCelebrityImagesList(params.id).then((result) => {
  ...
  function deleteImage(code) {
    console.log(code)
    celebritiesService.deleteImage(code).then((result) => {
      if (result) {
        updateData()
      }
    })
    ...
    <CCol xs={12}>
      <CCard className="mb-4">
        <CCardHeader>
          ...
          <br></br>
          <CButton
            onClick={() => navigate("/celebrity/post-images-add/"+params.id)}
            color={'primary'}
          >
