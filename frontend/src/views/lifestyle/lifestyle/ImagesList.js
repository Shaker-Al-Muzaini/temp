...
const ImagesList = () => {
  ...
  function updateData() {
    lifestyleService.getLifestyleImagesList(params.id).then((result) => {
  ...
  function deleteImage(code) {
    console.log(code)
    lifestyleService.deleteImage(code).then((result) => {
      if (result) {
        updateData()
      }
    })
    ...
    <CCol xs={12}>
      <CCard className="mb-4">
        <CCardHeader>
          ...
          <br></br>
          <CButton
            onClick={() => navigate("/lifestyle/post-images-add/"+params.id)}
            color={'primary'}
          >
