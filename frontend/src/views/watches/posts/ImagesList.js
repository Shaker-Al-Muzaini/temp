...
const ImagesList = () => {
  ...
  function updateData() {
    watchesService.getWatchesImagesList(params.id).then((result) => {
  ...
  function deleteImage(code) {
    console.log(code)
    watchesService.deleteImage(code).then((result) => {
      if (result) {
        updateData()
      }
    })
    ...
    <CCol xs={12}>
      <CCard className="mb-4">
        <CCardHeader>
          ...
          <br></br>
          <CButton
            onClick={() => navigate("/watches/post-images-add/"+params.id)}
            color={'primary'}
          >
