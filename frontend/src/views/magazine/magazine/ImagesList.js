...
const ImagesList = () => {
  ...
  function updateData() {
    magazinesService.getMagazineImagesList(params.id).then((result) => {
  ...
  function deleteImage(code) {
    console.log(code)
    magazinesService.deleteImage(code).then((result) => {
      if (result) {
        updateData()
      }
    })
    ...
    <CCol xs={12}>
      <CCard className="mb-4">
        <CCardHeader>
          ...
          <br></br>
          <CButton
            onClick={() => navigate("/magazine/post-images-add/"+params.id)}
            color={'primary'}
          >
